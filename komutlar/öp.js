...
exports.run = function(client, message, args) {
    ...
    if (!args[0]) {
        const ghembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setTitle(`${message.author.username} kendisini öptü :O (aslında çok garip)`)
    ...
    if (!message.mentions.members.first().user.username === message.isMentioned(message.author)) {
        const hembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setTitle(`${message.mentions.members.first().user.username}, ${message.author.username} tarafından öpüldü! Ne tatlı ama ^^`)
